...
TEST(TestClsSDK, EndToEndWithPartitioning) {
  ...
  builder->Project(std::vector<std::string>{"tip_amount", "fare_amount"}); // how to project on the partition col ?
  auto filter = ("VendorID"_ == int64_t(2) && "payment_type"_ == int64_t(4)).Copy();
