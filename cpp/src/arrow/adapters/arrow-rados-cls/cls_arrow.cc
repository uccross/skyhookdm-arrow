...
static int scan(cls_method_context_t hctx, ceph::buffer::list* in,
  ...
  arrow_ret = arrow::dataset::DeserializeScanRequestFromBufferlist(&filter, &schema, *in);
  if (!arrow_ret.ok()) {
  ...
  arrow_ret = arrow::dataset::ExtractBatchesFromBufferlist(&batches, bl);
  if (!arrow_ret.ok()) {
  ...
  arrow_ret = arrow::dataset::ScanBatches(filter, schema, batches, &result_table);
  if (!arrow_ret.ok()) {
  ...
  arrow_ret = arrow::dataset::SerializeTableToBufferlist(result_table, result_bl);
  if (!arrow_ret.ok()) {
