...
Result<std::shared_ptr<Dataset>> RadosDataset::Make(RadosDatasetFactoryOptions factory_option) {

  auto cluster = std::make_shared<RadosCluster>(factory_option.pool_name_, factory_option.ceph_config_path_);
...
Result<RecordBatchIterator> RadosScanTask::Execute() {
  ...
  ARROW_RETURN_NOT_OK(SerializeScanRequestToBufferlist(options_->filter,
                                                       options_->projector.schema(), in));
